...
def remove_file_db(file: File):
    ...
    return (
        ...
        and remove_directory(base)
        and remove_directory(os.path.join(constants_.STATIC_M3U8_PATH, sha256_hex[0:2], sha256_hex))
    )
...
def get_extra_file_hash(path: str) -> tuple:
    ...
    return (h_md5.digest(), h_sha1.digest(), h_sha512.digest())
...
def tempfile(
    mode="w",
    ext=".mp4",
    temp_folder=constants_.STATIC_TEMP_PATH,
    replace_into=None,
    open_=True,
):
