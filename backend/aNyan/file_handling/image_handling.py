...
def make_clip_rect_fit(image_res: Tuple[int, int], clip_rect: Tuple[int, int, int, int]):
...
def normalise_PIL_image_to_RGB(pil_image: PIL_Image.Image):
    ...
    if pil_image.mode != desired_mode:
        ...
        if pil_image.mode == "LAB":
            ...
            pil_image = PIL_Image_Cms.profileToProfile(
                pil_image,
...
def generate_numpy_image(path: str, mime: int, force_pil: bool = False) -> numpy.array:
...
def generate_thumbnail_bytes_from_static_image_path(
    path: str,
...
def get_thumbnail_resolution_and_clip_region(
    ...
    if thumbnail_scale_type in (THUMBNAIL_SCALE_DOWN_ONLY, THUMBNAIL_SCALE_TO_FIT):
        ...
        return None, (max(int(thumbnail_width), 1), max(int(thumbnail_height), 1))
    ...
    return clip_rect, (max(int(thumbnail_width), 1), max(int(thumbnail_height), 1))


def generate_save_image_thumbnail(
    source_path: str,
    dest_path: str,
    mime: int,
    ...
    (clip_rect, target_resolution) = get_thumbnail_resolution_and_clip_region(image_size, thumbnail_size, thumbnail_scale_type)

    LOGGER.info(
        "Determined image size: {}. Target thumb resoltuion: {} for {}".format(image_size, target_resolution, source_path)
    )
