...
def parse_ffmpeg_video_line(lines, png_ok=False):
    ...
    lines_video = [
        l for l in lines if search(r"^\s*Stream", l) and "Video: " in l and True not in (bvf in l for bvf in bad_video_formats)
    ]
...
def extract_subs(video_path, output_folder):
    ...
    ffp_args = [
        ...
        "s",
        "-show_entries",
        "stream=index:stream=codec_type:stream_tags=language",
        "-of",
        "csv=p=0",
        video_path,
    ]
...
def encode_video_standard(video_path: str, output_path: str):
    ...
    ff_args = [
        ...
        "2",
        "-c:s",
        "mov_text",  # only sub format mp4 container supports
        "-r",
        "23.976",
        "-g",
        "48",
