...
try:
    ...
    logging.critical(
        "image-size-reader could not be found, this module is required, install it from https://github.com/Minnowo/image-size-reader"
    )
...
def _stream_video(video_name: str, request: Request):
    ...
    read_to = min(
        int(_range.group("max") or seek_to + constants_.VIDEO_STREAM_CHUNK_SIZE),
        file_size,
    )
    ...
    try:
        ...
        if mime == constants_.mime_types.UNDETERMINED_VIDEO:  # video
            ...
            if not file_handling.video_handling.generate_thumbnail(src, dst, constants_.THUMBNAIL_SIZE):
                ...
                logging.warning("Failed to generate thumbnail for: {}".format(src))

        elif not file_handling.image_handling.generate_save_image_thumbnail(
            src,
            dst,
            mime,
        ...
        logging.warning("FFMPEG exception while generating thumbnail for {}. -> {}".format(src, e))

    except Exception as e:

        logging.error("Error while generating thumbnail for {}.".format(src), e, exc_info=True)
    ...
    try:
        ...
        if "tags" in kwargs:
            ...
            for t in kwargs["tags"]:
                ...
                logging.info("Adding tag '{}' to file: '{}'".format(t, sha256.hex()))
                ...
                try:
                    ...
                    logging.error("Exception thrown adding tag to file ", e, exc_info=True)

    except HTTPException as e:

        logging.warning("Exception thrown adding file to the database -> {}".format(e))
...
def process_video(data):
    ...
    try:
        ...
        logging.error(
            "KeyError while fetching from data dict in process_video {}".format(e),
            stack_info=True,
        )
        return
    ...
    try:
        ...
        logging.warning("Exception extracting subtitles from video {}".format(e), stack_info=True)
...
async def process_file_upload(file: UploadFile):
    ...
    if mime == MT.UNDETERMINED_VIDEO or mime in constants_.VIDEO_MIMES:
        ...
        if mime == MT.APPLICATION_UNKNOWN:
            ...
            logging.warning("unknown video, ffmpeg could not determine video format -> assuming invalid file")
    ...
    if not file_ext:
        ...
        logging.warning("could not find a file extension for mime type of '{}'".format(mime))
    ...
    if not util.rename_file(tempname, filename):
        logging.warning(
            "Cannot rename file '{0}' -> '{1}' hash '{2}' assuming file exists".format(tempname, filename, sha256_hex)
        )

    kwargs = {"tags": ["filename:" + original_filename]}
    ...
    if not util.create_directory(ts_folder):
        ...
        logging.warning("Cannot create ts folder '{0}' for video file '{1}'".format(ts_folder, filename))
