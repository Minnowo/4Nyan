...
def Get_Temp_Path(suffix: str = "", dir: str = None) -> tuple[int, str]:
    ...
    return tempfile.mkstemp(suffix=suffix, prefix=NyanConstants.BRAND, dir=dir)
...
def get_temp_dir(dir: str = None) -> Union[str, bytes]:
    ...
    return tempfile.mkdtemp(prefix=NyanConstants.BRAND, dir=dir)
...
def clean_up_temp_path(os_file_handle, temp_path: str):
    ...
    try:
        ...
        with TEMP_PATH_LOCK:
            ...
            IN_USE_TEMP_PATHS.add((NyanData.time_now(), temp_path))
...
def clean_up_old_temp_paths():
    ...
    with TEMP_PATH_LOCK:
        ...
        for row in data:
            ...
            if NyanData.time_has_passed(time_failed + 60):
                ...
                try:
                    ...
                    if NyanData.time_has_passed(time_failed + 600):
...
def set_env_temp_dir(path: str):
    ...
    try:
        ...
        NyanPaths.make_sure_directory_exists(path)

    except Exception as e:

        raise Exception("Could not create the temp dir: {}".format(e))

    if not NyanPaths.directory_is_writeable(path):
